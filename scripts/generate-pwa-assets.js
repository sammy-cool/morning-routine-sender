...
class Canvas2D {
      ...
      for (let px = x0; px < x1; px++) {
        ...
        if (inside) {
          if (strokeColor && edgeDist <= strokeWidth) {
            this.setPixel(px, py, strokeColor[0], strokeColor[1], strokeColor[2], strokeColor[3] || 255);
          } else if (fillColor) {
  ...
  drawText(text, x, y, scale = 1, [r, g, b, a = 255]) {
    const GLYPHS = {
      ...
      'G': [[1,0],[2,0],[3,0],[0,1],[0,2],[0,3],[2,3],[3,3],[1,4],[2,4],[3,4],[3,2]],
      ...
      'X': [[0,0],[3,0],[0,1],[3,1],[1,2],[2,2],[0,3],[3,3],[0,4],[3,4]],
      'Y': [[0,0],[3,0],[0,1],[3,1],[1,2],[2,2],[1,3],[1,4]],
...
function generateBrandIcon() {
  ...
  canvas.fillRoundedRect(pad, pad, cardSize, cardSize, 96, [17, 24, 39, 230], [99, 102, 241, 200], 3);
...
function generateDesktopScreenshot() {
  ...
  canvas.drawText('CURATED DEEP WORK SPRINT COMPANION & HABIT STREAK TRACKER', 32, 158, 1.8, [148, 163, 184, 255]);

  const leftX = 32, leftW = 680;
  ...
  tasks.forEach((t, i) => {
    const ty = 490 + i * 48;
    canvas.fillRoundedRect(leftX + 20, ty, leftW - 40, 40, 10, t.active ? [99, 102, 241, 30] : [15, 23, 42, 120], t.active ? [99, 102, 241, 150] : [255, 255, 255, 15], 1);
    canvas.fillRoundedRect(leftX + 32, ty + 10, 20, 20, 5, t.done ? [16, 185, 129, 255] : (t.active ? [99, 102, 241, 200] : [255, 255, 255, 30]));
    if (t.done) {
      canvas.drawText('✓', leftX + 36, ty + 12, 1.4, [255, 255, 255, 255]);
    }
    canvas.drawText(t.label, leftX + 64, ty + 14, 1.4, t.done ? [148, 163, 184, 255] : (t.active ? [255, 255, 255, 255] : [203, 213, 225, 255]));
  });

  // RIGHT COLUMN: Timer & Soundscapes
  const rightX = 730, rightW = 518;
  ...
  canvas.fillRoundedRect(rightX + 20, 235, rightW - 40, 95, 14, [0, 0, 0, 150], [99, 102, 241, 50], 1);
  ...
  canvas.fillRoundedRect(rightX + 348, 345, 150, 42, 10, [255, 255, 255, 25], [255, 255, 255, 35], 1);
  canvas.drawText('RESET', rightX + 398, 358, 1.4, [255, 255, 255, 255]);

  canvas.drawText('AUTOMATIC HABIT STREAK LOGGING ON SPRINT COMPLETION', rightX + 45, 405, 1.2, [148, 163, 184, 255]);
  ...
  canvas.drawEqualizer(rightX + rightW - 100, 492, 4, 3, [8, 16, 22, 12, 18, 6], [52, 211, 153, 255]);
...
function generateMobileScreenshot() {
  ...
  mobileTasks.forEach((t, i) => {
    const ty = 515 + i * 50;
    canvas.fillRoundedRect(44, ty, W - 88, 42, 10, t.active ? [99, 102, 241, 35] : [15, 23, 42, 140], t.active ? [99, 102, 241, 180] : [255, 255, 255, 15], 1);
    canvas.fillRoundedRect(56, ty + 10, 22, 22, 6, t.done ? [16, 185, 129, 255] : (t.active ? [99, 102, 241, 220] : [255, 255, 255, 25]));
    if (t.done) {
      canvas.drawText('✓', 61, ty + 13, 1.4, [255, 255, 255, 255]);
    }
    canvas.drawText(t.label, 90, ty + 14, 1.6, t.done ? [148, 163, 184, 255] : (t.active ? [255, 255, 255, 255] : [203, 213, 225, 255]));
  });
  ...
  tabs.forEach((tab, i) => {
    ...
    canvas.drawText(tab.label, tabX, 1270, 1.5, tab.active ? [255, 255, 255, 255] : [148, 163, 184, 255]);
  });
